...
1. Phase 1：建立性能基线
2. Phase 2：syntax 低风险提速
3. 重新跑基线并比较收益
4. 如果 resolver 成为主要瓶颈，再进入 Phase 3
5. 如果仍不够，再讨论 Phase 4
...
- 新增 benchmark/diagnostic 入口
- 跑完小/中/大 3 组样例
- 将结果补回文档

### Milestone B: Syntax 提速

- `StepTokenizer` 支持区间扫描
- `StepParser` 去掉 `toUpperCase()` / `substring()`
- 回跑 syntax、resolver、全量测试
- 更新 benchmark 结果

### Milestone C: Resolver 优化决策

- 基于 benchmark 判断是否进入 resolver 优化
- 若 `resolve` 已非主瓶颈，则暂停，不做无效重构

### Milestone D: Resolver 热点收敛
...
近期最值得做的，不是继续直接改 resolver，而是：

1. 先补 benchmark/diagnostic 入口
2. 再完成 syntax 层低风险提速
3. 最后用数据决定是否继续下探 resolver
