...
## 模块实体类型数量统计（OCCT 8.0.0-rc5）

| 模块 | 头文件数 | 注册实体数 | 主要内容 |
|------|----------|------------|----------|
| **StepBasic** | 148 | ~150 | 基础数据、单位、日期、组织、产品定义 |
| **StepVisual** | 143 | ~70 | 可视化、样式、颜色、标注、网格化 |
| **StepShape** | 105 | ~105 | 拓扑实体（B-Rep、实体、面、边、壳） |
| **StepGeom** | 97 | ~90 | 几何实体（曲线、曲面、B样条、变换） |
| **StepKinematics** | 86 | ~65 | 运动学（运动副、机构表示） |
| **StepRepr** | 85 | ~85 | 表示实体（形状表示、映射项、材料） |
| **StepFEA** | 67 | ~67 | 有限元分析（节点、单元、材料属性） |
| **StepDimTol** | 62 | ~55 | GD&T 公差实体 |
| **StepAP214** | 46 | ~46 | AP214 特有实体 |
| **StepElement** | 37 | ~37 | 有限元元素描述符 |
| **StepAP203** | 21 | ~21 | AP203 特有实体 |
...
OCCT 8.0.0-rc5 支持 **713 个活跃注册的 STEP 实体类型**，覆盖：
- AP203（配置控制设计）
- AP214（汽车设计，多版本）
- AP242（管理模型工程）
...
**rc5 与 rc3 对比**：实体注册数量相同（713 个），但模块头文件数量进行了优化精简，整体代码更加紧凑。