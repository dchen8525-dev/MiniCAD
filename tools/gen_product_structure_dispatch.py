#!/usr/bin/env python3
"""Fold StepDumpApp.validateProductStructureEntity's 29-branch sequential-if
chain into a table-driven dispatch (PRODUCT_STRUCTURE_RULES).
...
This script edits the source in place:
  ...
  3. writes the frozen type order to
     src/test/resources/product-structure-dispatch-order.txt.

Idempotent: aborts if PRODUCT_STRUCTURE_RULES already exists.
...
ORDER_TXT = ROOT / "src/test/resources/product-structure-dispatch-order.txt"

TABLE_FIELD = "PRODUCT_STRUCTURE_RULES"
METHOD_SIG = "    private static Integer validateProductStructureEntity("
INSERT_BEFORE = METHOD_SIG
RULE_RECORD = "ProductStructureRule"
RULE_HANDLER = "ProductStructureHandler"
RULE_FACTORY = "productStructureRule"
RESULT_TYPE = "Integer"
...
TABLE_HEADER = [
    ...
    "    }",
    "",
    "    private static final List<ProductStructureRule> "
    + TABLE_FIELD
    + " = List.of(",
]
