#!/usr/bin/env python3
"""1:1 body-fidelity check for the StepSolidBuilder.buildSolid fold.

buildSolid is an instance-method dispatch returning `Solid`. The generated lambdas carry
`self` (StepSolidBuilder) + the dispatch var `entity` + the method param `id`, and are
...
def extract_table_entries(text):
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    marker = "private static final List<SolidBuilderRule> " + TABLE_FIELD + " = List.of("
    ...
    i = 0
    while True:
        j = region.find("solidBuilderRule(", i)
        if j == -1:
            break
        depth = 0
        k = j
        while k < len(region):
            ...
            k += 1
        m = re.search(
            r"solidBuilderRule\(([\w.]+)\.class,\s*\(self,\s*entity,\s*id\)\s*->\s*\{(.*)\}\s*\)\s*,?$",
            entry, re.S)
        if not m:
...
def main():
    ...
    if ok:
        print(f"FAITHFUL: all {len(orig)} branch bodies match verbatim (StepSolidBuilder.buildSolid, self.-stripped + fall-through normalised)")
        sys.exit(0)
    sys.exit(1)
