#!/usr/bin/env python3
"""1:1 body-fidelity check for the collectStandaloneEdges table-driven fold.

collectStandaloneEdges is a static void recursive collector. The generated lambdas are
in static context (no `self`), so each generated lambda body equals the original branch
body VERBATIM up to whitespace -- the lambda simply carries the method's params
(item, edges, resolved, builder, metadata) which the original body already referenced by
those exact names. This verifier proves that:

  1. the original sequential-if chain (from git HEAD) and the generated EDGE_COLLECT_RULES
     table have the SAME type sequence (OR-compound branches split into per-type entries
     on BOTH sides), and
  2. every branch body matches verbatim (whitespace-normalized).

Exits non-zero on any divergence. This is the behavioral-equivalence proof BEFORE the
build runs.
"""
import re
import subprocess
...
def extract_table_entries(text):
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    marker = "private static final List<EdgeCollectRule> " + TABLE_FIELD + " = List.of("
    ...
    i = 0
    while True:
        j = region.find("edgeCollectRule(", i)
        if j == -1:
            break
        depth = 0
        k = j
        while k < len(region):
            ...
            if c in "({":
                ...
                if depth == 0:
                    ...
                    break
            k += 1
        m = re.search(r"edgeCollectRule\(([\w.]+)\.class,\s*\(item,\s*edges,\s*resolved,\s*builder,\s*metadata\)\s*->\s*\{(.*)\}\s*\)\s*,?$", entry, re.S)
        if not m:
