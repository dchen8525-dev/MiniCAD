#!/usr/bin/env python3
"""Fold StepCadSolidBuilder.buildSolid's ~40-branch sequential `if (entity instanceof ...)` chain
into a table-driven dispatch (SOLID_RULES).
...
first-match-return dispatch:
  ...
  - Therefore the table uses the §5 NULL-FALLTHROUGH loop: each handler returns a (nullable)
    ...
    the loop.
  - Because it is an instance method, the handler carries `self` (StepCadSolidBuilder) plus the
...
Edits the source in place (idempotent: aborts if SOLID_RULES already present).
"""
import re
...
ORDER_TXT = ROOT / "src/test/resources/solid-dispatch-order.txt"
...
def main() -> None:
    ...
    for type_list, body, is_guarded in branches:
        ...
        for type_name in type_list:
            ...
            seen.add(s)
            entries.append(
                "        solidRule(%s.class, (self, entity, id) -> {\n%s\n        })" % (type_name, transformed)
            )
            order.append(s)
    ...
    TABLE_HEADER = [
        ...
        "    }",
        "",
        "    private static final List<SolidRule> " + TABLE_FIELD + " = List.of(",
    ]
