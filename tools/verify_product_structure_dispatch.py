#!/usr/bin/env python3
"""1:1 body-fidelity check for the validateProductStructureEntity fold.

validateProductStructureEntity is a `private static` first-match-return chain,
...
chain from git HEAD and the generated PRODUCT_STRUCTURE_RULES table from the
...
def extract_table_entries(text):
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    marker = (
        "private static final List<ProductStructureRule> " + TABLE_FIELD + " = List.of("
    )
    ...
    i = 0
    while True:
        j = region.find("productStructureRule(", i)
        if j == -1:
            break
        depth = 0
        k = j
        while k < len(region):
            ...
            k += 1
        m = re.search(
            r"productStructureRule\((\w+)\.class,\s*\(entity,\s*builder\)\s*->\s*\{(.*)\}\s*\)\s*,?$",
            entry,
            re.S,
        )
        if not m:
...
def main():
    ...
    if ok:
        print(
            ...
            "(validateProductStructureEntity)"
        )
        sys.exit(0)
    sys.exit(1)
