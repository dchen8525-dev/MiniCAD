#!/usr/bin/env python3
"""1:1 body-fidelity check for the buildCurve2 table-driven fold.

buildCurve2 is an instance method, so the generated lambdas must reach instance
state through `self.` (e.g. `self.buildLine2`, `self.buildCurve2`,
`self.entitiesById`). That means the generated table body is NOT verbatim-equal to
the original branch body -- every instance call carries a `self.` prefix.

This verifier proves behavioral equivalence anyway: it pulls the original
sequential-if chain from git HEAD and the generated BUILD_CURVE2_RULES table from
...
def extract_table_entries(text):
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    marker = "private static final List<CadCurve2Rule> " + TABLE_FIELD + " = List.of("
        ...
        while k < len(region):
            ...
            if c in "({":
                ...
                if depth == 0:
                    ...
                    break
            k += 1
        m = re.search(r"cadCurve2Rule\((\w+)\.class,\s*\(self,\s*item\)\s*->\s*\{(.*)\}\s*\)\s*,?$", entry, re.S)
        if not m:
...
def main():
    ...
    for idx, (ob, gb) in enumerate(zip(orig, gen), 1):
        ...
        if o_type != g_type:
            ...
            continue
        if o_guarded:
            # buildCurve2 has no guarded branches; flag if one appears unexpectedly.
    ...
    if ok:
        print(f"FAITHFUL: all {len(orig)} branch bodies match verbatim (buildCurve2, self.-stripped)")
        sys.exit(0)
    sys.exit(1)
